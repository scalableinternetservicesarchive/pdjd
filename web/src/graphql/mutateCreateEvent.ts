...
import {
  createNewEvent, createNewEventVariables
} from './query.gen'

const createEventMutation= gql`mutation createNewEvent (
  $eventTitle:String!
  $eventDesc:String!,
  $eventStartTime:Date!,
  $eventEndTime:Date!,
...
export function create_new_event(client:ApolloClient<any>,event_input:createNewEventVariables){
  return  client.mutate<createNewEvent, createNewEventVariables>({
    ...
    variables:{eventTitle:event_input.eventTitle,eventDesc:event_input.eventDesc
    ,eventStartTime:event_input.eventStartTime,eventEndTime:event_input.eventEndTime
